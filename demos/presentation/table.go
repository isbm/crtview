...
const tableBasic = `[green]func[white] [yellow]main[white]() {
    table := crtview.[yellow]NewTable[white]().
    ...
    [yellow]for[white] row := [red]0[white]; row < [red]40[white]; row++ {
        [yellow]for[white] column := [red]0[white]; column < [red]7[white]; column++ {
            ...
            align := crtview.AlignLeft
            [yellow]if[white] row == [red]0[white] {
                align = crtview.AlignCenter
            } [yellow]else[white] [yellow]if[white] column == [red]0[white] || column >= [red]4[white] {
                align = crtview.AlignRight
            }
...
func Table(nextSlide func()) (title string, content crtview.Primitive) {
	...
	for row, line := range strings.Split(tableData, "\n") {
		for column, cell := range strings.Split(line, "|") {
			...
			tableCell := crtview.NewTableCell(cell)
	...
	separator := func() {
		table.SetBorders(false)
		table.SetSelectable(false, false)
		table.SetSeparator(crtview.Borders.Vertical)
		code.Clear()
	...
	for _, tableText := range demoTableText {
		item := crtview.NewListItem(tableText.text)
