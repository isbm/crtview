...
func main() {
	...
	form.AddButton("保存", func() {
		_, option := form.GetFormItem(0).(*crtview.DropDown).GetCurrentOption()
		userName := form.GetFormItem(1).(*crtview.InputField).GetText()
...
func alert(panels *crtview.Panels, id string, message string) {
