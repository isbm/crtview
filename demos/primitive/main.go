...
func (r *RadioButtons) Draw(screen tcell.Screen) {
	...
	for index, option := range r.options {
		...
		crtview.Print(screen, []byte(line), x, y+index, width, crtview.AlignLeft, tcell.ColorYellow.TrueColor())
	}
}

// InputHandler returns the handler for this primitive.
func (r *RadioButtons) InputHandler() func(event *tcell.EventKey, setFocus func(p crtview.Primitive)) {
	return r.WrapInputHandler(func(event *tcell.EventKey, setFocus func(p crtview.Primitive)) {
		switch event.Key() {
		case tcell.KeyUp:
			r.currentOption--
